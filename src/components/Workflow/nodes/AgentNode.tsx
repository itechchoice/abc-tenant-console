...
import type { NodeStatus } from '@/components/Workflow/WorkflowCanvas/graph';

type AgentNodeType = 'analysis' | 'plan' | 'tool' | 'decision' | 'handoff' | 'response' | 'model';
...
const TYPE_CONFIG: Record<AgentNodeType, TypeConfigEntry> = {
  analysis: {
    icon: ScanSearch,
...
function StatusIcon({ status, typeIcon: TypeIcon, typeIconClass }: StatusIconProps) {
  if (status === 'running') {
