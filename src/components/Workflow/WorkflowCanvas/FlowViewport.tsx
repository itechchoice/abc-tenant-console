...
export function FlowViewport({ graph, fitViewTrigger }: FlowViewportProps) {
  ...
  const handleNodeClick: NodeMouseHandler<Node<AgentNodeData>> = useCallback((_event, node) => {
    const latestStep = [...steps].reverse().find((step) => step.nodeKey === node.id);
    if (latestStep) selectStep(latestStep.id);
  }, [selectStep, steps]);

  const handleMoveStart = useCallback((_event: unknown, viewport: unknown) => {
    if (programmaticMoveRef.current || !viewport) return;
    setFollowLive(false);
  }, [setFollowLive]);
