...
export function inferNodeKeyFromStepName(
...
function createStepTitle(stepName: string | null | undefined, chatMode: ChatMode = 'auto') {
...
function defaultStepDetail(nodeKey: WorkflowNodeKey, chatMode: ChatMode = 'auto') {
  if (chatMode === 'model') {
...
function getEntryNode(chatMode: ChatMode = 'auto'): WorkflowNodeKey {
  return chatMode === 'model' ? 'prompt' : 'understand';
}

function getResponseNode(chatMode: ChatMode = 'auto'): WorkflowNodeKey {
...
function buildSummary(
  ...
  let finalState: string = 'workflow';
  if (status === 'failed') {
    finalState = 'failed';
...
interface WorkflowRuntimeState {
  ...
  resultSummary: WorkflowExecutionSummary | null;
  lastCompletedSummary: WorkflowExecutionSummary | null;
...
interface RecordToolCallPayload {
  messageId: string;
  toolName?: string | null;
  args?: unknown;
  taskId?: string | null;
...
interface WorkflowRuntimeActions {
  startExecution: (payload?: StartExecutionPayload) => void;
  syncExecutionContext: (payload: {
    sessionId?: string | null;
    taskId?: string | null;
    workflowId?: string | null;
  }) => void;
  recordStep: (payload?: RecordStepPayload) => string | null;
  recordToolCall: (payload: RecordToolCallPayload) => string | null;
  markAwaitingInteraction: (payload: MarkAwaitingPayload) => string | null;
  finishExecution: (status: Extract<WorkflowExecutionStatus, 'completed' | 'failed'>) => void;
  selectStep: (stepId: string | null) => void;
  selectStepByMessageId: (messageId: string) => void;
  setFollowLive: (flag: boolean) => void;
  restoreLastExecution: () => void;
  resetRuntime: () => void;
}

type WorkflowRuntimeStore = WorkflowRuntimeState & WorkflowRuntimeActions;
...
export const useWorkflowRuntimeStore = create<WorkflowRuntimeStore>((set, get) => ({
  ...INITIAL_STATE,
  ...
  recordStep: (payload = {}) => {
    ...
    set((current) => {
      ...
      return {
        phase: 'live' as const,
        status: nextStep.status === 'waiting' ? 'waiting' as const : 'running' as const,
        chatMode,
        steps: mergedSteps,
  ...
  finishExecution: (status) => {
    ...
    set((state) => {
      const now = Date.now();
      const steps: WorkflowRuntimeStep[] = state.steps.map((step) => {
        if (step.status === 'running' || step.status === 'pending') {
          return { ...step, status: 'completed' as const, endedAt: now };
        }
        if (status === 'failed' && step.status === 'waiting') {
          return { ...step, status: 'failed' as const, endedAt: now };
        }
        return step;
      });
