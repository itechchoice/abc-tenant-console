...
更新 `definition` 后 `version` 自动加 1。若工作流当前为 `published` 状态，更新 `definition` 会自动将状态回退为 `draft`（需重新发布才可执行）。
...
将工作流状态从 `draft` 变更为 `published`。只有已发布的工作流才能被执行。
...
|400|[Bad Request](https://tools.ietf.org/html/rfc7231#section-6.5.1)|工作流无节点或已删除|Inline|
|404|[Not Found](https://tools.ietf.org/html/rfc7231#section-6.5.4)|工作流不存在|Inline|
...
将工作流状态从 `published` 恢复为 `draft`。取消发布后工作流不可再被执行。
...
|状态码|状态码含义|说明|数据模型|
|---|---|---|---|
|200|[OK](https://tools.ietf.org/html/rfc7231#section-6.3.1)|取消发布成功|Inline|
|400|[Bad Request](https://tools.ietf.org/html/rfc7231#section-6.5.1)|工作流当前不是 published 状态|Inline|
|404|[Not Found](https://tools.ietf.org/html/rfc7231#section-6.5.4)|工作流不存在|Inline|
...
测试运行工作流，允许 `draft` 和 `published` 状态的工作流执行。内部执行逻辑与正式运行完全一致，区别仅在于不要求 `published` 状态。适用于开发调试场景。
...
|message|body|string| 否 |输入消息|
|modelId|body|string| 否 |指定模型ID|
...
|状态码|状态码含义|说明|数据模型|
|---|---|---|---|
|200|[OK](https://tools.ietf.org/html/rfc7231#section-6.3.1)|测试运行启动成功|Inline|
|400|[Bad Request](https://tools.ietf.org/html/rfc7231#section-6.5.1)|工作流已删除|Inline|
|404|[Not Found](https://tools.ietf.org/html/rfc7231#section-6.5.4)|工作流不存在|Inline|
...
**前置条件：工作流状态必须为 `published`，草稿或已删除的工作流不可执行。**
...
|状态|说明|
|---|---|
|`draft`|草稿状态，创建后默认状态。可编辑、不可执行。|
|`published`|已发布状态。可执行（通过 Task 或 Webhook 触发）。|
|`deleted`|已删除（软删除）。不可编辑、不可执行、不在列表中显示。|
...
published → draft  (POST /{id}/unpublish 或更新 definition 时自动回退)
draft/published → deleted  (DELETE /{id})
```

### 执行限制

- 只有 `published` 状态的工作流才能通过 `POST /tasks`（capabilities）、`POST /workflows/{id}/run` 或 `POST /trigger/webhook` 执行
- 草稿状态的工作流尝试执行会返回 400 错误
